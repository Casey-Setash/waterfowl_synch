...
# ICC_surv <- tau.year.betat^2 / ((tau.year.betat^2) + (tau.year^2))
  ICC_surv <- sig.year.betat^2 / ((sig.year.betat^2) + (sig.year^2)) #CMS changed from tau to sig because tau is the precision and sig is the SD (which we're then squaring here to become the variance)
...
# Priors and constraints
 ...
 eta.strata.sigma ~ dunif(0,5)
 eta.yr.sigma ~ dunif(0,5)
 eta.site.tau <- pow(eta.site.sigma,-2)
 eta.strata.tau <- pow(eta.strata.sigma,-2)
 eta.yr.tau <- pow(eta.yr.sigma,-2)

 for (t in 1:n.years){ # where t indexes to number of years surveyed 
 eta.yr[t] ~ dnorm(0,eta.yr.tau) # generate random year effects across strata
 } # close first year loop
 ...
 eta.strata[s,t] ~ dnorm(0,eta.strata.tau) # generate random year effects within each stratum
...
# ICC_fec <- eta.yr.tau^2 / ((eta.yr.tau^2) + (eta.site.tau^2) + (eta.strata.tau^2))
  ICC_fec <- eta.yr.sigma^2 / ((eta.yr.sigma^2) + (eta.strata.sigma^2)) 
  
  # CMS changed these back to sigma so that the ICC calculation is using variance rather than precision
  # correct me if I'm wrong here, I tried to double check in Ch 16 of the textbook, which uses variance (they're just putting the priors on precision, which is how you had it before sorry)
  # also should we only include stratum-level temporal variability here (eta.strata.sigma^2) and not site-level as well? That way the ICC calculation is the same as the one for survival
...
# using different indexing now to skip years with no releases
...
# 2.3.1 Observation model
 ...
 logit(Pjuv[i]) <- b0[stratum[i]] + b1.mu*Lat[i] + b2.mu*Long[i] + eta.yr[yr[i]] +
                   eta.strata[stratum[i],yr[i]] + eta.site[stratum[i],yr[i],site[i]] 
 ...
 for (t in 1:n.years){ # where t indexes to number of years surveyed in each stratum
    logit(Pjuv.strat[s,t]) <- b0[s] + eta.yr[t] + eta.strata[s,t] 
...
dat<-merge(dat, strat, by="STRAT", all.y=TRUE) #STRAT is the original stratum number that appears on FWS BPOP maps, stratno is renamed to start at 1
for(k in 2:5){
...
library(tidyverse)
dat %>% select('year','STRAT') %>% ftable() # shows you how many banding sites per stratum in each year
...
n1 <- y[,1] # ECM. Old CS comment: use the count data to make this the initial pop size of adult females 
...
# abundance
jags.data <- list(n1=n1, y=y, y.sds=y.sds,  
                  
                  # survival
                  n.years = ncol(marrAHYf)-1, marrAHYf = marrAHYf, 
                  relAHYf = relAHYf, n.strata=ncol(relAHYf),
                  stratum=dat$stratno, 
                  yr=as.numeric(as.factor(dat$year)),  
                  rel_a_ind=rel_a_ind, rel_a_ind_n=rel_a_ind_n,
...
plotRhats(nopi_ipm) # I love this function - what a nice treat!
...
plot(x=seq(1:years), y=nopi_ipm$mean$Nfl[1,], type='b', pch=16, ylab='f',
     xlab=NA, xlim=c(0,years), axes=FALSE, col=col_vector[1], cex=ptc)
